...
function genAccessToken(ID) {
	...
	logger.debug("jwt expire: " + config.jwtExpiration );
    let tok= jwt.sign( {session: ID}, process.env.TOKEN_SECRET, { expiresIn: '1800h'  });
	logger.debug("gen tok: " + tok);
	var decoded = jwt.decode(tok);
	logger.debug("iat : " + decoded.iat);
	logger.debug("exp : " + decoded.exp);
	let date = new Date()
	date.setUTCSeconds(decoded.exp);
	logger.debug(" exp date? " + date.toDateString());
	return tok;
}
...
function extractToken(req) {
	logger.debug("extract token starts");
		  fs.writeFile('postgetprofilehdr.txt', JSON.stringify(req.headers), function (err) {
		if (err) throw err;
		logger.debug('post getprofile hdr file is created successfully.');
		});

	let thisHdr = JSON.stringify(req.headers);
	logger.debug("thisHdr: " + thisHdr);
	...
	logger.debug('extracted token: ' + accessToken);
	return(accessToken);
};

exports.verifyToken = (req, res, next) => {
	logger.debug('start verifytoken');
  try {
	const token = extractToken(req);
	logger.debug('extracted token exp: ' + jwt.decode(token).exp); 
	var dateexp = new Date(jwt.decode(token).exp);
	logger.debug('exp date? : ' + dateexp);
	logger.debug('extracted token iat: ' + jwt.decode(token).iat);
	logger.debug('token exp - iat diff: ' + (jwt.decode(token).exp - jwt.decode(token).iat));
    logger.debug("decoded: " + JSON.stringify(jwt.decode(token)));
    const decoded = jwt.verify(token, process.env.TOKEN_SECRET);
    next();
  }
  catch(ex){
	  logger.debug("verifyToken error, dammit: " + ex);
    res.sendStatus(401);
}};
...
//	if (!accessToken) {
//			logger.debug('accessToken err 401');
//			res.sendStatus(401);
...
//	logger.debug(" decoded req token string: " + JSON.stringify(reqToken));
//	logger.debug(" req token exp time: " + reqToken.exp);
//	jwt.verify(accessToken, process.env.TOKEN_SECRET, function(err, decoded) {
//		logger.debug(" decode err: " + err);
...
//
//logger.debug("now find, sessionID from user: " + req.body.username);
//
//	var storedSessionID;
//	
//	Session.findOne({ 'username': req.body.username }, 'id_ sessionid authtoken username', 
//		function (err, session) {
//			logger.debug("type of session: " + typeof(session));
//    if (err) {
...
//	logger.debug('storedSessionID: ' + storedSessionID);
//	logger.debug('session stuff: ' + session._id + ' ' + session.authtoken + ' ' + session.username);
//	});
//
//	let newToken = genAccessToken(storedSessionID);
...
//		logger.debug(" now save this stuff to session");
//	};
