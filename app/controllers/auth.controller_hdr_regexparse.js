...
// regex	"accessToken=(([A-Za-z0-9-_=]+\.[A-Za-z0-9-_=]+\.?[A-Za-z0-9-_.+/=]*))
	...
	var daToken = thisHdr.match(/"accessToken=(([A-Za-z0-9-_=]+\.[A-Za-z0-9-_=]+\.?[A-Za-z0-9-_.+/=]*))/)[1];
	logger.debug('da token: ' + daToken);
	logger.debug("whole header string: " + thisHdr);
...
logger.debug("now, thisID: " + thisID);
    ...
    if (err) {
		...
		// result
	logger.debug('session stuff: ' + session._id + ' ' + session.sessionid + ' ' + 
