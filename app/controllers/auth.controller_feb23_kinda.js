...
exports.test = (req, res) => {
	logger.debug("in controller.test" );
	return("No session");
};

exports.something = (req) => {
	logger.debug('in something, req.body: ' + req.body);
	return("did it");
};
...
    	else{
			if (!session) {
				// if this ever shows up... weird.
				return("Session Not Found, No Refresh");
			};
	       	logger.debug("Original session : ", session.sessionid);
    	}
	});
