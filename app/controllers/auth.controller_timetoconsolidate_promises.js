...
//	jwt.verify(reqToken, process.env.TOKEN_SECRET);
    ...
    if (err) {
		...
		logger.debug("maybe found session");
		if (! session) {
    ...
    .then((result) => {
      if (result) {
		  logger.debug("update session done");
        //return Session.findById(id);
      } else {
		  logger.debug("findand update error?");
...
//      res.status(200).json(updatedDocument);
	...
	    	}
	    	else{
				logger.debug("update session past else");
		    	}
			logger.debug("update returning next");
			next();
		});
	logger.debug("verifyToken FINDUPDATE  CALL END");
...
exports.saveProfile = (req, res) => {
	logger.debug("controller saveProfile");
};
